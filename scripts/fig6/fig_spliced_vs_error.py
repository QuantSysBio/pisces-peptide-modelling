...
SPLICED_FOLDERS = ['output/spliced721_241206']
ERROR_PATHS = ['predictedScores.csv']
ys = []
for s_f, c_f in zip(SPLICED_FOLDERS, ERROR_PATHS):
    unique_spliced_df = pd.read_csv(
        f'{s_f}/unique_peps_scored.csv', usecols=['peptide', 'label', 'meanScore']
    )
    ...
    mm_df = pd.concat([unique_cryptic_df, unique_spliced_df])
    print(mm_df.shape)
